...
tidy data: every column has its own variable, every row has an observation, every cell has a value.
...
```
Use quotes to tell R that lakeid and annnumber are not objects but instead variables in the dataframe. 
...
There are NAs. When I take an average of max. life, I get a negative number because the data contains negative values when measuring the max. life. 
```{r}
anyNA(mammals)
```
...
order, family, genus, species, mass, gestation, newborn, weaning, wean mass, AFR, maximum life, litter size and litters per year. 
```{r}
names(mammals)
```
...
```{r}
mammals_new <- rename(mammals, genus="Genus", afr="AFR", litters_year="litters/year", wean_mass="wean mass", max_life="max. life", litter_size="litter size")
```
