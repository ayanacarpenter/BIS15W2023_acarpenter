...
```{r}
#is.na(msleep) not the most practical
```
...
There are NAs present and they are represented as -999

```{r}
amniota <- readr::read_csv("data/amniota.csv") #can pipe here and add na_if("-999") if youre familiar with the data and know how NAs are represented 
```
...
```{r}
naniar::miss_var_summary(amniota_tidy) #summary of missing data
```
...
This result seems unlikely because this is a big data set. But it is because NAs are represented as -999
```{r}
