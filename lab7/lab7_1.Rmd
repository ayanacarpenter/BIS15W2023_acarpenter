...
penguins %>% 
  ...
  summarize(n_penguins=n(), .groups = 'keep') #the keep removes the error
```
...
```{r}
penguins %>% 
  count(island, species) #long data format
```
...
```{r}
penguins %>% 
  tabyl(island, species) #wide data format, not tidy because species are observations and not variable names
```
...
There are species with their sex listed as NA on all three islands. 
```{r}
penguins %>% 
