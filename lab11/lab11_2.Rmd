...
```{r}
my_palette <- paletteer_d("fishualize::Hypoplectrus_puella")
```
...
homerange %>% 
  ...
  labs(title = "Mass vs. Homerange across taxonomic class",
       x = "Mass",
       y = "Homerange")
...
homerange %>% 
  ...
  labs(title = "Trophic Guild vs. Taxonomic Class",
       x = "Trophic Guild",
       y = "Class")
```
...
## Practice
1. Build a faceted plot that shows a distribution of mass by habitat type.
```{r}
ecosphere %>% 
  ggplot(aes(x= log10_mass)) + 
  geom_density() + 
