...
homerange %>% 
  ...
  geom_histogram(alpha = 0.5, color = "black", fill = "rosybrown4", bins=40)+
...
homerange %>% 
  ...
  geom_density(color = "lemonchiffon")+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.5, color = "black", fill = "rosybrown4")+
...
```{r}
homerange_new %>% 
  select(range_category, class, log10.mass) %>% 
  filter(range_category=="small") %>% 
  ggplot(aes(x=class, y=log10.mass, fill = class)) + geom_boxplot() + 
  labs(title = "Masses of each taxonomic class across small range species",
       x = "Taxonomic class",
       y= "Log10.Mass")
```
