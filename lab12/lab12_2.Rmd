...
homerange %>% 
  ...
  geom_density(fill="rosybrown4", alpha=0.6)
```
```{r}
names(homerange)
```
...
ui <- fluidPage(
  ...
  selectInput("x", "Select Taxon", choices = c("mammals", "birds", "lake fishes", "river fishes", "lizards", "turtles", "tortoises", "marine fishes", "snakes"), 
              selected = "mammals"), 
  plotOutput("plot", width ="600px", height = "500px")
)
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    homerange %>% 
      ...
      ggplot(aes(x=log10.hra)) + 
      geom_density(fill="rosybrown4", color = "black", alpha=0.9)
  })
