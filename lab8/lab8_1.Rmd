...
Each religion's income is not its own observation row. So you have to give a name to the incomes and add a column of the total number of people in the religion who receive that income. ***The column names are data!

3. Use `pivot_longer()` to make the data tidy.  
```{r}
relig_income %>% 
  pivot_longer(-religion, #religion will not pivot
               names_to = "income",
               values_to = "n_total"
               )
```
...
Each greenhouse is a piece of data, each water schedule prog is a piece of data and each day is a piece of data. (Variable names are data).
